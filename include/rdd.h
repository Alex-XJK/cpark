...
public:
  ...
  constexpr size_t splits_num() const { return splits_; }
 ...
 */
template <Rdd R, typename Func, typename T = std::invoke_result_t<Func, std::ranges::range_value_t<R>>>
requires std::invocable<Func, std::ranges::range_value_t<R>>&&
    std::convertible_to<std::invoke_result_t<Func, std::ranges::range_value_t<R>>, T> class TransformedRdd {
public:
  constexpr TransformedRdd(R prev, Func func)
      : prev_{std::move(prev)}, func_{std::move(func)}, split_ranges_(splits_), splits_{prev.splits_num()} {
  ...
  constexpr auto get_split(size_t i) const { return split_ranges_[i]; }
...
public:
  ...
  requires std::invocable<Func, T, T>&& std::convertible_to<std::invoke_result_t<Func, T, T>, T>&&
    ...
    auto results =
        std::ranges::subrange(futures) | std::views::transform([](std::future<T>& fut) { return fut.get(); });
